...
describe('Photo e2e test', () => {
    ...
    it('should load Photos', async () => {
        ...
        expect(await photoComponentsPage.getTitle()).to.eq('budenzauberApp.photo.home.title');
    });

    it('should load create Photo page', async () => {
        await photoComponentsPage.clickOnCreateButton();
        photoUpdatePage = new PhotoUpdatePage();
        expect(await photoUpdatePage.getPageTitle()).to.eq('budenzauberApp.photo.home.createOrEditLabel');
    ...
    it('should create and save Photos', async () => {
        ...
        await promise.all([
            ...
            photoUpdatePage.eventSelectLastOption()
            // photoUpdatePage.tagSelectLastOption(),
        ]);
        expect(await photoUpdatePage.getTitleInput()).to.eq('title');
    ...
    it('should delete last Photo', async () => {
        ...
        expect(await photoDeleteDialog.getDialogTitle()).to.eq('budenzauberApp.photo.delete.question');
